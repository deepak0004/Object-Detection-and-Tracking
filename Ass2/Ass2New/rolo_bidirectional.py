...
class ROLO():
	...
	def __init__(self):
		...
		self.rolo_weights_dir = "/home/sharat/Documents/assignment2_new/ROLO/weights/trainNew"
		self.rolo_weights_intermediate_dir = "/home/sharat/Documents/assignment2_new/ROLO/weights/trainNew/intermediate"
		...
		self.num_videos = 30
		self.offset = 0
		self.num_epoches = 1000 #self.num_videos * 100
	...
	def createPlaceholders(self):
		...
		self.state_fw = tf.placeholder(tf.float32, [None, 2 * self.yolo_output_size])
		self.state_bw = tf.placeholder(tf.float32, [None, 2 * self.yolo_output_size])
	...
	def model(self):
		...
		lstm_fw = tf.nn.rnn_cell.LSTMCell(self.yolo_output_size, self.yolo_output_size, state_is_tuple = False)
		lstm_bw = tf.nn.rnn_cell.LSTMCell(self.yolo_output_size, self.yolo_output_size, state_is_tuple = False)
		
		istate_forward = self.state_fw
		istate_backward = self.state_bw

		for time_step in xrange(self.time_steps):
			output, istate_forward, istate_backward = tf.contrib.rnn.static_bidirectional_rnn(lstm_fw, lstm_bw, [Xi[time_step]], istate_forward, istate_backward)
			if time_step == 0:
				output_backward = output[0][0][self.yolo_output_size:]
			if time_step == self.time_steps - 1:
				output_forward = output[0][0][:self.yolo_output_size]
			tf.get_variable_scope().reuse_variables()

		#print(output_forward)
		#print(output_backward)
		self.lstm_output = tf.multiply(tf.add(output_backward, output_forward), 0.5)
		#print(self.lstm_output)
		self.box_prediction = self.lstm_output[4097:4101]
		self.box_prediction = tf.reshape(self.box_prediction, [-1, self.gt_size])
		#print(self.box_prediction)
	...
	def trainModel(self):
		...
		with tf.Session(config = self.config) as sess:
			...
			for epoch in xrange(self.num_epoches):
				...
				img_w, img_h, video_seq, t, train_iters = helper.getVideoSequenceMetadata(video_id)
				...
				while index < train_iters - self.time_steps:
					...
					input_dict = {self.x: inputs_x, self.y: inputs_y, self.state_fw: np.zeros((self.batch_size, 2 * self.yolo_output_size)), self.state_bw: np.zeros((self.batch_size, 2 * self.yolo_output_size))}

...
class ROLO():
	...
	def __init__(self):
		...
		self.rolo_weights_dir = "/home/sharat/Documents/assignment2_new/ROLO/weights/trainHeatMap"
		self.rolo_weights_intermediate_dir = "/home/sharat/Documents/assignment2_new/ROLO/weights/trainHeatMap/intermediate"
	...
	def createPlaceholders(self):
		...
		self.y = tf.placeholder(tf.float32, [None, self.predict_size])
	...
	def model(self):
		...
		self.heatmap_prediction = self.lstm_output[0][:, self.feature_size:self.yolo_output_size]


	def loss(self):
		loss_sqr = tf.square(self.heatmap_prediction - self.y)
	...
	def trainModel(self):
		...
		with tf.Session(config = self.config) as sess:
			...
			for epoch in xrange(self.num_epoches):
				...
				img_w, img_h, video_seq, _, train_iters = helper.getVideoSequenceMetadata(video_id)
				
				x_dir = os.path.join("DATA", video_seq, "yolo_heat")
				y_dir = os.path.join("DATA", video_seq, "groundtruth_rect.txt")
				out_dir = os.path.join("DATA", video_seq, "rolo_heat_train")
				...
				while index < train_iters - self.time_steps:
					...
					inputs_y = helper.locToHeatmapCoordinates(inputs_y[0])
					inputs_y = [helper.heatmapCoordinatesToHeatmapVector(inputs_y)]
	...
	def testModel(self, video_id, out_dir = "rolo_out_train"):
		with tf.Session(config = self.config) as sess:
			...
			while index < test_iters - self.time_steps:
				...
				heatmap_prediction = sess.run(self.heatmap_prediction, feed_dict = input_dict)
				heatmap_prediction = helper.heatmapVectorToHeatMap(heatmap_prediction)

				helper.dumpRoloPredictions(heatmap_prediction, index, self.time_steps, self.batch_size, out_dir)

				if index % self.display_count == 0:
					loss = sess.run(self.loss, feed_dict = input_dict)
					print("Index: {}, Loss: {}, Video Sequence Name: {}".format(index, "{:.6f}".format(loss), video_seq))
					total_loss += loss
